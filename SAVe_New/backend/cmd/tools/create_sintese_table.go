...
func main() {
	...
	// Add foreign key
	// Note: GORM AutoMigrate might not add FKs automatically depending on driver,
	// but let's try to add it explicitly if needed or rely on the fact it's 1:1 with ID_Caso

	// Check if table exists
	if database.DB.Migrator().HasTable(&SAVe_SinteseAnalitica{}) {
	...
	fmt.Println("✅ Foreign Key constraint added (if not exists).")
}
